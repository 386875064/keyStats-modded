...
> ## ⚠️ Unofficial modified build / 非官方修改版
>
> This repository is a **third-party modified version** of [keyStats](https://github.com/debugtheworldbot/keyStats)
> (original author **pipizhu**, MIT License). It is **not** the official release — the original author
> neither maintains nor endorses it.
>
> 本仓库是 [keyStats](https://github.com/debugtheworldbot/keyStats) 的**非官方修改版**（原作者 **pipizhu**，MIT 许可）。
> 与官方版本无关，原作者不提供支持或背书。
>
> ### What is different / 改了什么
>
> | | |
> |---|---|
> | 🎮 **LoL match stats** | Auto-detects a match, records keys/clicks/APM per game, in-game HUD, per-minute timeline, key heatmap<br>英雄联盟对局统计：自动开局、逐局记录、游戏内 HUD、每分钟时间轴、键盘热力图 |
> | 🛠 **Dual capture channel** | Raw Input (`RIDEV_INPUTSINK`) primary + low-level hook fallback, so anti-cheat detaching the hook no longer zeroes out the numbers<br>双采集通道：Raw Input 为主、钩子兜底，解决游戏内钩子被反作弊摘除导致统计归零 |
> | 📊 **Usage insights** | Backspace rate, focus/app-switching, hourly activity, sitting reminder<br>使用洞察：退格率、专注度、活跃时段分布、久坐提醒 |
> | 🔌 **Fully offline** | Cloud sync removed from UI and never started; analytics disabled and the upstream key purged. Verified to make zero network calls<br>纯离线：同步入口已移除且服务不启动，分析上报已停用，实测零网络请求 |
>
> 👉 **Full description / 详细说明：[README-MODIFIED.md](./README-MODIFIED.md)**
> 📄 **License & attribution / 许可与署名：[NOTICE.md](./NOTICE.md)**
