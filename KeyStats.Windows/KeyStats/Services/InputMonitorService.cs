...
public class InputMonitorService : IDisposable
{
    ...
    private long _hookReinstallCount;
    ...
    /// <summary>True while the low-level keyboard/mouse hooks are installed.</summary>
    public bool IsMonitoring => _isMonitoring;

    /// <summary>
    /// How many times the hooks had to be reinstalled because Windows silently dropped
    /// them. A rising counter during a game session usually means the game's anti-cheat
    /// is detaching third-party hooks.
    /// </summary>
    public long HookReinstallCount => Interlocked.Read(ref _hookReinstallCount);
    ...
    private void ReinstallHooks()
    {
        ...
        StartHookThread();
        ResetTransientState();
        Interlocked.Increment(ref _hookReinstallCount);
